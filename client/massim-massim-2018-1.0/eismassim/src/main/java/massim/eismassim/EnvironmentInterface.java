...
public class EnvironmentInterface extends EIDefaultImpl implements Runnable
{
	 ...
	 *
	 */
	private static final long		serialVersionUID	= -7676944794211821450L;
	 ...
	 *
	 * @param configFile
	 *            the actual path to the config file (including the file name)
	 */
	...
	protected LinkedList<Percept> getAllPerceptsFromEntity(String name) throws PerceiveException, NoEnvironmentException
	{
		...
		if (e == null)
			throw new PerceiveException("unknown entity");
		if (!e.isConnected())
	 ...
	 *             in case configuration is invalid and no reasonable standard value can be assumed
	 */
	private void parseConfig() throws ParseException
	{
		...
		for (int i = 0; i < jsonEntities.length(); i++)
		{
			...
			if (name == null)
				throw new ParseException("Entity must have a valid name", 0);
			String username = jsonEntity.optString("username");
			if (username == null)
				throw new ParseException("Entity must have a valid username", 0);
			String password = jsonEntity.optString("password");
			if (password == null)
				throw new ParseException("Entity must have a valid password", 0);
	...
	public void run()
	{
		while (this.getState() != EnvironmentState.KILLED)
		{
			...
			{} // be nice to the server and our CPU
		}
	}
	 ...
	 * @param percepts
	 *            the percepts to send notifications for
	 */
