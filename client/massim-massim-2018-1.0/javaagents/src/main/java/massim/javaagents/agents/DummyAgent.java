...
public class DummyAgent extends Agent
{
	...
	public Action step()
	{
		...
		for (Percept percept : percepts)
		{
			switch (percept.getName())
			{
				...
				case "charge":
					...
					break;
				case "minLat":
					minLat = getDoubleParam(percept, 0);
					break;
				case "maxLat":
					maxLat = getDoubleParam(percept, 0);
					break;
				case "minLon":
					minLon = getDoubleParam(percept, 0);
					break;
				case "maxLon":
					maxLon = getDoubleParam(percept, 0);
					break;
				case "lat":
					lat = getDoubleParam(percept, 0);
					break;
				case "lon":
					lon = getDoubleParam(percept, 0);
					break;
				case "lastAction":
					lastAction = getStringParam(percept, 0);
					break;
				case "lastActionResult":
					lastActionResult = getStringParam(percept, 0);
					break;
				case "wellType":
					...
					break;
				case "massium":
					massium = getIntParam(percept, 0);
					break;
				case "score":
					score = getIntParam(percept, 0);
					break;
				case "chargingStation":
					chargingStations.put(getStringParam(percept, 0), percept);
					break;
				case "step":
					step = getIntParam(percept, 0);
					break;
			}
		}
		...
		if (leader.equals(getName()))
			say("Score: " + score + " Massium: " + massium);
	...
	private double getDoubleParam(Percept percept, int position)
	{
		Parameter p = percept.getParameters().get(position);
		if (p instanceof Numeral)
			return ((Numeral) p).getValue().doubleValue();
		return 0;
	}
